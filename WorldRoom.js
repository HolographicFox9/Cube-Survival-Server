...
export class WorldRoom extends Room {
  ...
  onCreate() {
    ...
    this.petDeathTimers=new Map(); this.solidGrid=new Map(); this.dynamicGrid=new Map(); this.chestRewards=new Map(); this.waveTimer=4;
    this.generateWorld();
  ...
  updateProjectiles(dt){
    const projectileKinds=new Set(["enemy","animal"]);
    for(const[id,p]of Array.from(this.state.projectiles.entries())){
      ...
      if(p.hostile){
        for(const[pid,pl]of this.state.players){
          if(pl.dead)continue;
          if(dist(p.x,p.y,pl.x,pl.y)<p.r+PLAYER_R){this.damageTarget({kind:"player",id:pid},p.dmg,"projectile",p.ownerId);hit=true;break;}
        }
        if(!hit){
          for(const[petId,pet]of this.state.pets){
            if(pet.dead)continue;
            if(dist(p.x,p.y,pet.x,pet.y)<p.r+pet.r*.75){this.damageTarget({kind:"pet",id:petId},p.dmg,"projectile",p.ownerId);hit=true;break;}
          }
        }
      }else{
        // Friendly projectiles can hit other players (PvP), then nearby hostile
        // creatures. The dynamic grid avoids a full-world scan for every arrow.
        for(const[pid,pl]of this.state.players){
          if(pid===p.ownerId||pl.dead)continue;
          if(dist(p.x,p.y,pl.x,pl.y)<p.r+PLAYER_R){
            this.damageTarget({kind:"player",id:pid},p.dmg,"playerProjectile",p.ownerId);
            this.broadcastFx({kind:"hit",x:pl.x,y:pl.y-8,text:Math.round(p.dmg),color:"#8fd4ff"});
            hit=true;break;
          }
        }
        if(!hit){
          for(const rec of this.nearbyDynamic(p.x,p.y,150,projectileKinds)){
            const o=rec.obj;if(!o)continue;
            if(rec.kind==="enemy"){
              if(dist(p.x,p.y,o.x,o.y)<p.r+o.r){
                this.hitEnemy(rec.id,o,p.dmg,p.ownerId);
                if(p.knock&&this.state.enemies.has(rec.id)){const a=Math.atan2(p.vy,p.vx);o.x+=Math.cos(a)*p.knock;o.y+=Math.sin(a)*p.knock;}
                hit=true;break;
              }
            }else if(animalProjectileTouch(o,p.x,p.y,p.r)){
              this.hitWild(rec.id,o,p.dmg,p.ownerId);
              if(p.knock&&this.state.animals.has(rec.id)){const q=Math.atan2(p.vy,p.vx);o.x+=Math.cos(q)*p.knock;o.y+=Math.sin(q)*p.knock;}
              hit=true;break;
            }
          }
        }
      ...
      if(!hit){
        for(const s of this.nearbySolids(p.x,p.y,32)){
          if(s.kind==="chest"&&this.state.chests.get(s.id)?.opened)continue;
          if(s.kind==="gold"&&!this.state.gold.get(s.id)?.infinite&&this.state.gold.get(s.id)?.goldLeft<=0)continue;
          if(dist(p.x,p.y,s.x,s.y)<p.r+s.r*.9){hit=true;break;}
        }
      }
